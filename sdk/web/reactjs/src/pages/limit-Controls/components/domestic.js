...
export function Domestic({ domesticPreferences }) {
  ...
  // Derive constant values from domesticPreferences
  const limitConfig = domesticPreferences ? domesticPreferences.limitConfig : [];
  ...
  const [transactions, setTransactions] = useState(() => {
    if (domesticPreferences) {
      setOnlineDefaultLimit(parseInt(domesticPreferences.limitConfig.find(config => config.txnType === "ECOM")?.dailyLimitValue));
      setOfflineDefaultLimit(parseInt(domesticPreferences.limitConfig.find(config => config.txnType === "POS")?.dailyLimitValue));

      return {
        online: { enabled: domesticPreferences.ecom, limit: onlineDefaultLimit },
        offline: { enabled: domesticPreferences.pos, limit: offlineDefaultLimit },
        allowATMLimit: domesticPreferences.atm,
        contactless: domesticPreferences.contactless,
      };
    } else {
      return {
